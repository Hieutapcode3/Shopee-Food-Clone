...
document.addEventListener("DOMContentLoaded", function () {
  var customCheckboxes = document.querySelectorAll(
  ...
  deleteTags.forEach(function (deleteTag) {
    deleteTag.addEventListener("click", function (event) {
      ...
      var checkboxes = underItem.querySelectorAll(
        ".custom-checkbox input[type='checkbox']"
      );
...
function updateUnderItems() {
  ...
  itemFilters.forEach(function (itemFilter) {
    ...
    var checkboxesInFilter = itemFilter.querySelectorAll(
      ".custom-checkbox input[type='checkbox']"
    );
    var checkedCheckboxes = Array.from(checkboxesInFilter).filter(
      (cb) => cb.checked
    );
